...
QString MailObject::getuserid() const{ // функция для получения заголовка фото
    return m_userid;
}

QString MailObject::gettextp() const{ // функция для получения описания фото
    return m_textp;
}

QUrl MailObject::getPhoto() const{ // функция для получения фото
    return m_photo;
}

int MailObject::getcommentscount() const{ // функция для получения размера фото
